...
def upload_file():
    ...
    if file and file.filename.lower().endswith(('.png', '.jpg', '.jpeg')):
        ...
        try:
            ...
            prompt = """
            このグラフ画像を解析してください。
            このグラフには横軸（X軸）と縦軸（Y軸）があり、目盛り線の一部にのみ数値ラベルが印字されています。

            【重要】
- グラフ上に実際に数字として印字されているラベルのみを抽出してください。
- 印字されていない目盛り線の数値を、間隔から推測・補完して出力してはいけません。
- 例えば目盛り線が等間隔にあっても、数字が書かれていない目盛りは無視してください。

【手順】
1. 横軸（X軸、グラフ下部）に実際に印字されている数字を左から右の順に抽出する。
2. 縦軸（Y軸、グラフ左側）に実際に印字されている数字を下から上の順に抽出する。
3. 軸のラベル名や単位（例: deg, Intensityなど）は除外し、純粋な数字のみとする。

【出力フォーマット】
以下のJSON形式のみで出力してください。説明文や前置きは一切不要です。
